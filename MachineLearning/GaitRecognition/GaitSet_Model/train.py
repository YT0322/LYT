...
batch_size = (4,6)  # 定义批次（2个人，每个人16个数据）24    default:p=8,k=16 128
...
if len(ckpfiles) > 1:
    modecpk = os.path.join(ckp, "best_-encoder.pt")
    optcpk = os.path.join(ckp, "best_-optimizer.pt")
...
for batch_data, batch_meta, batch_label in train_loader:
    ...
    # 将标签转化为张量
    target_label = np2var(np.array(batch_label)).float()     # p*k个标签
    ...
    if triplet_loss.hard_or_full == 'full':  # 提取损失值
        ...
        (full_loss_metric_, full_loss_num_,  mean_dist_) = triplet_loss(triplet_feature,triplet_label)
        # print(full_loss_metric_.shape)
        # print(full_loss_num_.shape)
        # print(mean_dist_.shape)

        loss = full_loss_metric_.mean()
        full_loss_metric.append(full_loss_metric_.mean().item())
        full_loss_num.append(full_loss_num_.mean().item())
    else:
        (hard_loss_metric_, mean_dist_) = triplet_loss(triplet_feature, triplet_label)
        loss = hard_loss_metric_.mean()
        hard_loss_metric.append(hard_loss_metric_.mean().item())

    trainloss.append(loss.item())  # 保存损失值
    dist_list.append(mean_dist_.mean().item())
